...
def test_boards_rank_a_poor_fit_lower_rather_than_dropping_him():
    ...
    # ⚠️ THE BAR IS DERIVED FROM THE PENALTY, not a hand-picked factor. A first version
    # asserted `vetGap > kidGap * 3`, which was sized against a 0.75 penalty and failed the
    # moment it was retuned to 0.90 (2.99 against a bar of 3) -- a test that breaks when a
    # constant it never mentions is tuned is measuring the constant, not the behaviour.
    # A 0.90 penalty is worth 1/0.90 - 1 of extra gap; the veteran must carry a real share
    # of it beyond whatever the Scouting Department is doing to both men.
    from managers.frontOfficeBrain import _SOFT_APPEAL_PENALTY, _SOFT_APPEAL
    vetGap = (plushBoard[2] - board[2]) / board[2]
    kidGap = abs(plushBoard[1] - board[1]) / board[1]
    penaltyGap = (1.0 / _SOFT_APPEAL_PENALTY) - 1.0 if _SOFT_APPEAL else 0.0
    assert vetGap - kidGap > penaltyGap * 0.5, (kidGap, vetGap, penaltyGap)
