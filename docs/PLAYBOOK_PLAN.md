...
- **RPO** (still to build): a pre-snap read that branches run/pass on the box (needs `getDefensiveScheme` lifted to `_executeWeightedPlay`).
...
Flea flicker, statue of liberty, reverse, halfback pass, double pass. **Fully templated by the QB scramble** (`_resolveQbScramble` ~`:10030`): flip `playType`, swap the ball-handler (RB-as-passer / WR-as-runner — stat methods are position-agnostic), set a flag, add a pre-empting `formatPlayText` branch. Injected as a **rare pre-roll** at the top of `playCaller` (like the clock-management checks), gated by `aggressiveness` + situation (a called shot, not late-desperation). High variance: big payoff if the defense bites (same deception hook as run concepts), disaster if sniffed out (execution).
