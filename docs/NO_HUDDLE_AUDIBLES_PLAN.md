...
5. **Guardrail** — league scoring and end-of-half points. ⚠️ The rating-multiplier → win-probability transfer is **1.619**, so a layer that looks small can move wins hard; measure before tuning.
...
4. **Audibles.** The read, the three outcomes. Measure 4.
5. **PBP + play insights** for both, so a reader can see it happen and a future investigation can read the decision.

Flags: `NO_HUDDLE_ENABLED`, `AUDIBLE_ENABLED`, both default-on once measured, so either can be switched off for an A/B without unpicking the other.
...
4. **Fatigue.** No-huddle should tire the offense faster. The sim has a fatigue model applied pre-game; a within-game tempo cost is a separate piece of work and is out of scope here.
