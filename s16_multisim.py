"""Season-16 multi-sim: resume from the latest prod snapshot N times under CURRENT
code, capture each run's S16 regular-season record, champion (Floos Bowl winner),
scoring, playoff berths, awakened counts.

Usage:
  run:    s16_multisim.py run <n> <parallel> <port_base> <out_json>
  report: s16_multisim.py report <out_json>
"""
import os, sys, time, json, shutil, subprocess, sqlite3
from collections import Counter

REPO = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(REPO, 'data', 'floosball_prod_latest.db')  # latest prod snapshot
...
def runBatch(n, parallel, portBase, outJson):
    workroot = f'/tmp/floo_s16_{portBase}'
...
def report(outJson):
    ...
    print(f"\n=== SEASON {SEASON} — {n} runs (from latest prod copy, current code) ===")
