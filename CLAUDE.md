...
- **MVP**: ballot = top `AWARD_MVP_BALLOT_SIZE` (5) players overall by `mvpScore` (not per-position). Single-vote plurality (ties → `mvpScore`). Resolved in `seasonManager._selectSeasonMVP` via `AwardsManager.resolveMvp`; below `AWARD_MVP_QUORUM` (3) distinct voters → top-`mvpScore` fallback. Window opens at regular-season end, closes at the season-end MVP announcement.
