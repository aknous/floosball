...
- **Offensive gameplan wiring** (`_applyGameplanMods`): the `OffensiveGameplan` object was historically almost all **display-only** — `runPassRatio` maps through `_runPassBias()` which is **never called**, and `passDepthDistribution` is never read; the only wired offensive lever is `gapDistribution` (run-gap selection, ~`:9638`). `_applyGameplanMods` is the FIRST offensive *passing* decision wired into play selection: it reads `OffensiveGameplan.passDepthBias` (scalar, +1 = full quick game) and biases the short/medium/long/deep weights. `adjustOffensiveGameplan` (gameplan.py) sets `passDepthBias` when the offense is STALLING (both run+pass below average), scaled by **`offensiveMind`** (a sharp coach dials into a quick, high-percentage passing game to move the chains; a poor one flails). **Situational**: suppressed in catch-up mode (behind 2+ scores in the 2nd half, or any deficit late — consts `QUICKGAME_SUPPRESS_DEFICIT`/`QUICKGAME_LATE_DEFICIT`) so the deep/desperation calling rides when chunk plays are actually needed. Defensive gameplan is similar: `coverageAssignments` is wired (who blitzes, ~`:9200`) but `blitzFrequency`/`runStopFocus`/`coverageTendency` are still display-only (wiring those is a tracked follow-up).
