...
- Play-calling chain: `playCaller()` → (clock-management checks first) → `_computePlayWeights()` → `_getBasePlayWeights()` (down/distance table) → `_applySituationalMods()` → `_applyMatchupMods()` → `_applyCoachMods()` → `_applyAwakenedMods()` → `_executeWeightedPlay()` → `_selectPassPlay()`. Separate `_fourthDownCaller()` and `_otPlayCaller()`.
